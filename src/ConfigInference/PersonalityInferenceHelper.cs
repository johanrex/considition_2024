...
namespace optimizer
{
    internal class PersonalityInferenceHelper
    {
        ...
        public static Dictionary<Personality, PersonalitySpecification> InferPersonalityInterestRatesBounds(ConfigService configService, ServerUtils serverUtils, Map map, string personalitiesFile)
        {
            var personalities = configService.GetPersonalitySpecifications(map.Name);
            ...
            foreach (var personalityEnum in personalities.Keys)
            {
                ...
                var gameInput = GameUtils.CreateSingleCustomerGameInput(map.Name, map.GameLengthInMonths, customer.Name, startInterestRate, monthsToPayBackLoan);
                var gameResponse = serverUtils.SubmitGameAsync(gameInput).Result;
                var score = gameResponse.Score.TotalScore;
                if (score <= 0)
                {
        ...
        private static double BoundedContinuousBinarySearch(ServerUtils serverUtils, Map map, Customer customer, double lowerBound, double upperBound, int monthsToPayBackLoan, bool findMax)
        {
            ...
            while (upperBound - lowerBound > 0.001)
            {
                ...
                var gameInput = GameUtils.CreateSingleCustomerGameInput(map.Name, map.GameLengthInMonths, customer.Name, mid, monthsToPayBackLoan);
                var gameResponse = serverUtils.SubmitGameAsync(gameInput).Result;
